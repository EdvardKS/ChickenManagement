            ...
            <Table className="w-full">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[15%] md:w-[15%] text-base md:text-lg text-center">Pollos</TableHead>
                  <TableHead className="w-[15%] md:w-[15%] text-base md:text-lg text-center">Hora</TableHead>
                  <TableHead className="hidden md:table-cell w-[0%] md:w-[30%] text-lg">Detalles</TableHead>
                  <TableHead className="w-[40%] md:w-[15%] text-base md:text-lg">Acciones</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {dateOrders.map((order, index) => (
                  <TableRow key={order.id} className={index % 2 === 0 ? "bg-gray-100" : "bg-white"}>
                    <TableCell className="w-[30%] md:w-[25%] text-base md:text-lg font-medium">
                      <div className="truncate max-w-[150px] md:max-w-full">
                        {order.customerName}
                      </div>
                      {/* Versión móvil/tablet: Muestra detalles compactos debajo del nombre */}
                      <div className="md:hidden text-xs text-gray-600 mt-1 max-w-[150px]">
                        {order.details ? order.details.substring(0, 30) + (order.details.length > 30 ? '...' : '') : '-'}
                      </div>
                    </TableCell>
                    <TableCell className="w-[15%] md:w-[15%] text-base md:text-lg text-center">
